        ...
        try:
            ...
            if is_host:
                ...
                if action == 'disable':
                    ...
                    # Start switch
                    device.start([])  # Empty controller list (sử dụng default)
                    logger.info(f"[EXECUTOR] Switch {device_name} started")
            ...
            # Verify kết quả bằng cách ping (cho host)
            verification = None
            if is_host and action == 'enable':
                # Thử ping để verify host đã up
                time.sleep(0.2)  # Wait cho interface lên
                # Tìm host khác để ping
...
# ========================================
# DEMO: TESTING (standalone mode)
# ========================================
if __name__ == "__main__":
    print("=" * 70)
    print("COMMAND EXECUTOR DEMO")
    print("=" * 70)
    print("\n⚠️  To test this, you need a running Mininet instance.")
    print("This demo shows the structure, not actual execution.\n")
    
    # Example command data structures
    examples = [
        {
            'name': 'Toggle Device - Disable h1',
            'data': {
                'action_id': 'act_001',
                'command': 'toggle_device',
                'data': {
