...
if __name__ == "__main__":
    print("=" * 70)
    print("TEST ACTION LOGGER SERVICE")
    print("=" * 70)
    
    # Tạo service
    logger_service = ActionLoggerService()
    
    # Test 1: Tạo action và mark thành công
    print("\n[TEST 1] Tạo action TOGGLE_DEVICE và mark SUCCESS")
    action1 = logger_service.create_action(
        ActionType.TOGGLE_DEVICE,
        "h1",
        {"action": "disable"}
    )
    print(f"Created: {action1}")
    
    import time
    time.sleep(0.5)
    
    logger_service.update_action(
        action1.action_id,
        ActionStatus.SUCCESS
    )
    print(f"Updated: {logger_service.get_action(action1.action_id).to_dict()}")
    
    # Test 2: Tạo action và mark thất bại
    print("\n[TEST 2] Tạo action UPDATE_LINK và mark FAILED")
    action2 = logger_service.create_action(
        ActionType.UPDATE_LINK,
        "h1-s1",
        {"bandwidth": 50, "delay": "10ms"}
    )
    ...
    logger_service.update_action(
        action2.action_id,
        ActionStatus.FAILED,
        error_message="Link h1-s1 not found"
    )
    
    # Test 3: Lấy lịch sử
    print("\n[TEST 3] Lấy lịch sử (limit=10)")
    history = logger_service.get_history(limit=10)
    for h in history:
        print(f"  - {h['action_id']}: {h['action_type']} | {h['status']}")
    
    # Test 4: Filter theo status
    print("\n[TEST 4] Lấy chỉ các actions FAILED")
    failed = logger_service.get_history(status_filter='FAILED')
    print(f"Total FAILED: {len(failed)}")
    
    # Test 5: Tạo nhiều actions
    print("\n[TEST 5] Tạo 5 actions để test pagination")
    for i in range(5):
        action = logger_service.create_action(
            ActionType.TOGGLE_LINK,
            f"h{i}-s1",
            {"action": "down"}
        )
        if i % 2 == 0:
            logger_service.update_action(action.action_id, ActionStatus.SUCCESS)
        time.sleep(0.1)
    
    print(f"Total actions: {logger_service.get_total_count()}")
    print(f"Total SUCCESS: {logger_service.get_total_count('SUCCESS')}")
    print(f"Total FAILED: {logger_service.get_total_count('FAILED')}")
    print(f"Total PENDING: {logger_service.get_total_count('PENDING')}")
    
    # Test 6: Pagination
    print("\n[TEST 6] Test pagination (limit=3, offset=2)")
    page = logger_service.get_history(limit=3, offset=2)
    for h in page:
        print(f"  - {h['action_id']}: {h['target']}")
